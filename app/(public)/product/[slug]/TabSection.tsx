...
const TabSection = ({ product }: TabSectionProps) => {
  ...
  const [shouldAutoOpenComposer, setShouldAutoOpenComposer] = useState(false);

  useEffect(() => {
    const handleOpenComments = () => {
      setActiveTab("comments");
      setShouldAutoOpenComposer(true);
    };

    window.addEventListener("open-product-comment", handleOpenComments);

    return () => {
      window.removeEventListener("open-product-comment", handleOpenComments);
    };
  }, []);

  return (
    <div id="product-comments" className="lg:col-span-9 space-y-8 scroll-mt-28">
      {/* Tabs Header */}
        ...
        {activeTab === "comments" && (
          <div className="animate-fade-in">
            <CommentsList
              ...
              autoOpenComposer={shouldAutoOpenComposer}
              onComposerOpened={() => setShouldAutoOpenComposer(false)}
            />
          </div>
        )}
      </div>

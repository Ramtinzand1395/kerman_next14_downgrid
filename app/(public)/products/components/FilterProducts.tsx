...
export default function FilterProducts() {
  ...
  const [mobileOpen, setMobileOpen] = useState(false);
  const [openedGroup, setOpenedGroup] = useState<string | null>(categories[0]?.slug ?? null);
  const selectedCategory = searchParams.get("category") || "";
  ...
  const selectedLabel =
    flatCategories.find((item) => item.slug === selectedCategory)?.name || "همه محصولات";
    ...
    <>
      ...
      <nav className="flex flex-col gap-2" aria-label="فیلتر دسته‌بندی محصولات">
        {categories.map((cat) => {
          ...
          return (
            <section key={cat.slug} className="rounded-xl border border-gray-100 p-2">
    ...
    <>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-black/40 md:hidden">
          ...
          <button type="button" className="h-full w-full" title="بستن" onClick={() => setMobileOpen(false)} />
        </div>
      )}
